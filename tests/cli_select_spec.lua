...
describe("cli select columns", function()
  ...
  it("prefers the layouts title over the pane label", function()
    local col = by_id(columns(agent({ tmux_layouts_title = "paperclip", tmux_pane_label = "panda" })))
    assert.matches("paperclip", text(col.label.parts))
  end)
